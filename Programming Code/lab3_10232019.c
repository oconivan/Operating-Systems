...
int k = 0; //File iterator
int j = 0; //fp iterator
char **filearray = NULL; // Name of the files created dynamically
char **pointersarray = NULL; //File pointers array
long double * catalanArray = NULL;
pthread_t * threadArray = NULL;
pthread_attr_t attr;
long double c=1; //catalan iterator
long int previous;
long int catalan_numbers;
long int thread_numbers;
int p; //thread iterator
...
int main(int argc, char **argv)
{
	...
	filearray = (char **)malloc(thread_numbers * sizeof(char *));
	pointersarray = (char **)malloc(thread_numbers * sizeof(char *));
	...
	{
		//printf("%s\n",filearray[j]);
		FILE **pointersarray = fopen(filearray[s], "rb");
	}
